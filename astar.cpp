...
const int IDIM = 35; // horizontal size of the squares
const int JDIM = 35; // vertical size size of the squares
...
class Node
{
    ...
    public:
		...
		Location getLocation()  {return Location(rPos,cPos);}
        int getGValue()  {return GValue;}
        int getFValue()  {return FValue;}
...
int* pathFind(Location locStart , Location locFinish )
				
{
	...
	// remove the node from the open list
	    ...
	    // if not wall (obstacle) nor in the closed list
			...
			d.getCell(iNext,jNext) !=3 && d.getCell(iNext,jNext) !=4   || closedNodes[iNext][jNext] <3)) {
